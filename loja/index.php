...
<style>
    .custom-img {
        width: 60%; /* Define a largura da imagem como 100% da largura do contêiner pai */
...
<body> 
 <div class="container">
    ...
    <div> 
        <br>
    <a class="btn btn-primary" href="registro.php">Cadastre-se</a> <a class="btn btn-primary" href="acesso.php">Login</a>
      <?php
        $carrinho = False; //false = sem carrinho -- true = carrino existe
        if(isset($_SESSION['nome_cliente'])){
            echo("<b>Seja Bem vindo, $nome_cliente</b>");
            $sql = "SELECT * FROM carrinho WHERE id_cliente = $id_cliente";
                ...
                while ($array = mysqli_fetch_array($busca_carrinho)){
                    ...
                    $carrinho = true; //achou o carrinho
                }
        }
    ?>
    <div>
    ...
    <div align="right">
        <?php if($carrinho == true){ ?>
            <div><b>Você possui ítens no carrinho!</b></div>
            <div><b>Clique no carrinho abaixo.<b></div>
            <a href="carrinho.php?id_carrinho=<?php echo($id_carrinho)?>"><img src="images/carrinho.png" width="60px"></a>
        <?php }?>
    </div>
    <br/>
    <br/>
